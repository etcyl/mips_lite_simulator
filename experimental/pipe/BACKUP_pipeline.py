    ...
    def simulation(self):
        ...
        while(self.PC<self.len_file or self.stop==1):#self.PC<self.len_file or self.stop==1):
            if clock_cycle == 1:
                ...
                self.ID(self.pipeline[0])
                self.IF(self.pipeline[0])
            elif clock_cycle == 3:
                self.EXE(self.pipeline[0])
                self.ID(self.pipeline[0])
                self.IF(self.pipeline[0])
            ...
            for i in range(len(self.pipeline)):
                if clock_cycle > 10:
                    break
                print(self.pipeline[i].get_rs())
                print(self.pipeline[i].get_rd())
                print(self.pipeline[i].get_rt())
                print(self.pipeline[i].get_ALU_result())
            clock_cycle += 1
